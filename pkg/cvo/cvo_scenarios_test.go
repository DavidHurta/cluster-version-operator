...
func TestCVO_MajorVersionManifestFiltering(t *testing.T) {
	...
	if worker.payload.MajorVersion == nil {
		t.Fatal("Expected payload to have MajorVersion set")
	}
	if *worker.payload.MajorVersion != 5 {
		t.Errorf("Expected MajorVersion to be 5, got %d", *worker.payload.MajorVersion)
	}
